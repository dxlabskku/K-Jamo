...
make_cpt_txt_if_needed() {
  if [[ -f "${DATA_DIR}/instruction_train.txt" ]]; then
    log "CPT txt exists: ${DATA_DIR}/instruction_train.txt"
    return 0
  fi
  log "Converting jsonl to CPT txt: ${DATA_DIR}/instruction_train.txt"
  "${PYTHON}" - << PY
import json
from pathlib import Path
inp = Path("${JAMO_SUBST_JSONL}")
out = Path("${DATA_DIR}/instruction_train.txt")
...
train_cpt_one() {
  ...
  if [[ "${RUN_CPT}" == "1" && ! -f "${out_dir}/config.json" ]]; then
    ...
    CUDA_VISIBLE_DEVICES="${TRAIN_GPUS}" torchrun --master_port "${MASTER_PORT}" --nproc_per_node="${NPROC}" train.py \
      ...
      --train_text "${DATA_DIR}/instruction_train.txt" \
  ...
  echo "[INFO] clean baseline; skip extended-tokenizer verification: ${out_dir}"
  echo "${out_dir}"
}
